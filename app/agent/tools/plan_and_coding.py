...
def plan_and_coding(raw_items: str, command: str, runtime: ToolRuntime) -> str:
    ...
    Args:
        raw_items: 编程任务的待办事项列表，每行一项，最多20项。如："检查并准备环境中的xxx、xxx包\n创建文件并写入代码\n运行代码"；当项目较大需要分多个文件或模块时，可以使用以下格式："检查并准备环境中的xxx、xxx包\n创建main.py并写入程序入口代码\n创建A_module/A.py文件并写入A模块的代码\n创建B_module/B_1.py文件并写入B_1模块的代码\n创建B_module/B_2.py文件并写入B_2模块的代码\n运行主程序代码"
